...
ipaddr = '192.168.2.9'
#ipaddr, port = input('App B started\nGive IP & port of directory (with whitespaces in between): \n').split()
port = input('App B started\nGive port of directory (with whitespaces in between): \n')
gl.grp_setdir(ipaddr, port)

gName, multicastIP, multicastPort, myID = input('App B: Give group name, multicastIP, multicastPort and your ID (with whitespaces in between): \n').split()

gsocket = gl.grp_join(gName, multicastIP, multicastPort, myID)

if gsocket == -1:
	print('ERROR: App B joined group!')
...
if messType == 'S':
	if len(msg) == 6:           #prokeitai gia arxeio ara dhmiourgia arxeiou
		...
		print("----")
		print("AppB: received message from: ",msg[0],"sender:", fileName, "file!")
		print("----")
		...
		print("----")
		print("AppB: received message from: ",msg[0],"sender:", data, " !")
		print("----")
elif messType == 'D':
	print("----")
	print("AppB: received message: ", msg)
	print("----")

time.sleep(15)

print('App B is leaving group...')
ret = gl.grp_leave(gsocket)
if ret == -1:
	print('ERROR: App B left group!')
